...
int main (int argc, char *argv[])
{
  ...
  // TcpDctcp connfection from N0 to N2
  ...
  // TcpDctcp connfection from N1 to N3

  uint16_t sinkPort3 = 5050;
  Address sinkLocalAddress3 (InetSocketAddress (i3i5.GetAddress (0), sinkPort3)); // interface of n2
  BulkSendHelper source3 ("ns3::TcpSocketFactory", sinkLocalAddress3);
  source3.SetAttribute("MaxBytes",UintegerValue (bytes));
  ApplicationContainer sourceApp3 = source3.Install(c.Get(1));
  sourceApp3.Start (Seconds (0.0));
  sourceApp3.Stop (Seconds (1.0));

  OnOffHelper clientHelper3 ("ns3::TcpSocketFactory", Address ());
  Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::TcpDctcp"));

  TypeId tid3 = TypeId::LookupByName ("ns3::TcpDctcp");
  std::stringstream nodeId3;
  nodeId3 << c.Get (0)->GetId ();
  std::string specificNode3 = "/NodeList/" + nodeId3.str () + "/$ns3::TcpL4Protocol/SocketType";
  Config::Set (specificNode3, TypeIdValue (tid3));
  Ptr<Socket> localSocket3 = Socket::CreateSocket (c.Get (3), TcpSocketFactory::GetTypeId ());

  PacketSinkHelper sinkHelper3 ("ns3::TcpSocketFactory", sinkLocalAddress3);
  ApplicationContainer sinkApp3 = sinkHelper3.Install (c.Get (3)); //n2 as sink
  sinkApp3.Start (Seconds (0.0));
  sinkApp3.Stop (Seconds (1.0));

  AsciiTraceHelper ascii;
  p2p.EnableAsciiAll(ascii.CreateFileStream("exp_4.tr"));

  AnimationInterface anim("exp_4.xml");
