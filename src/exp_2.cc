...
int main (int argc, char *argv[])
{
  ...
  // TcpBic connfection from N0 to N2
  ...
  // TcpBic connfection from N1 to N3
  Address sinkLocalAddress_1 (InetSocketAddress (i3i5.GetAddress (0), sinkPort));
  BulkSendHelper source1 ("ns3::TcpSocketFactory", sinkLocalAddress_1);
  source1.SetAttribute("MaxBytes",UintegerValue (bytes));
  ApplicationContainer sourceApp1 = source1.Install(c.Get(1));
  sourceApp1.Start (Seconds (0.0));
  sourceApp1.Stop (Seconds (1.0));

  OnOffHelper clientHelper1 ("ns3::TcpSocketFactory", Address ());
  Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::TcpBic"));

  TypeId tid1 = TypeId::LookupByName ("ns3::TcpBic");
  std::stringstream nodeId1;
  nodeId1 << c.Get (0)->GetId ();
  std::string specificNode1 = "/NodeList/" + nodeId1.str () + "/$ns3::TcpL4Protocol/SocketType";
  Config::Set (specificNode1, TypeIdValue (tid1));
  Ptr<Socket> localSocket1 = Socket::CreateSocket (c.Get (3), TcpSocketFactory::GetTypeId ());


  PacketSinkHelper sinkHelper1 ("ns3::TcpSocketFactory", sinkLocalAddress_1);
  ApplicationContainer sinkApp1 = sinkHelper1.Install (c.Get (3));
  sinkApp1.Start (Seconds (0.0));
  sinkApp1.Stop (Seconds (1.0));

  AsciiTraceHelper ascii;
  p2p.EnableAsciiAll(ascii.CreateFileStream("exp_2.tr"));

  AnimationInterface anim("exp_2.xml");
